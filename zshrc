...
function lt    { l -t -c "$@" } # sort by time
...
alias ..='cd ..;l'
alias cd..='cd ..'
alias rehash='. ~/.zshrc; . ~/.zshenv' # source ~/.zshrc after I edit it
...
# I've given up on Sublime and TextMate.
# So Atom ftw, unless not.
# In which case, nano forever!
...
#   note: -X keeps the output on screen when less quits. handy.
#         but be sure to set that -X in the LESS env var, lest we confuse
#         some poor souls (ie, rvm)
#         -R means we get colors instead of ESC junk.
export PAGER="less"
